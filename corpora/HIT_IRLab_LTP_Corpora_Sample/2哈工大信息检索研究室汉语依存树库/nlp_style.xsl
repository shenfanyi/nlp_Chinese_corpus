...
<xsl:template match="/">
		...
		if(isNE.checked) //输出命名实体识别信息
		{
			...
			for(var i = 0; i != wordNum; i++) 
			{
				...
				if(ne.search(/Nm/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:red" colspan="' + colspan + '">数词</td>';
				}
				else if(ne.search(/Ni/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:blue" colspan="' + colspan + '">机构名</td>';
				}
				else if(ne.search(/Ns/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:olive" colspan="' + colspan + '">地名</td>';
				}
				else if(ne.search(/Nh/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:green" colspan="' + colspan + '">人名</td>';
				}
				else if(ne.search(/Nt/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:orange" colspan="' + colspan + '">时间</td>';
				}
				else if(ne.search(/Nr/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:purple" colspan="' + colspan + '">日期</td>';
				}
				else if(ne.search(/Nz/) != -1)
				{
					...
					i = j-1;
					body += '<td bgcolor="lightyellow" style="color:brown" colspan="' + colspan + '">专名</td>';
				}
				else
				{
	...
	if(isParser.checked) //输出句法分析信息
	{
		...
		var texttop = maxWidth*8 + textheight + 40 + addHeight; //文字距上边的距离
...
<xsl:template match="xml4nlp">
	...
	<font color="red" size="2">意见和建议请发表在 <A target="_blank"><xsl:attribute name="HREF">./MessageBoard/index.asp<xsl:value-of select="Url"/></xsl:attribute><xsl:value-of select="ir.hit.edu.cn"/>HIT IR-Lab LTP 留言版</A></font>
	<hr />
	<font color="red"><b>选择显示内容：</b></font>
	<select name="item" value="origin" onchange="choose(this.value)">
		...
		<option value="coref">指代消解</option>
		<option value="sum">自动文摘</option>
		<option value="class">文本分类</option>
	</select>
	<!--
	<input type="submit" name="button" value="显示"/>
	-->
	<xsl:text>	</xsl:text>
	<input type="button" value="按句子显示" onclick="choose('all')"/>
	<xsl:text>	　　</xsl:text>
	<font color="red"><b>欢迎使用IRLab-NLPML系统</b></font>
	<xsl:text>	　　　</xsl:text>
	<input type = "button" value = "返回文本输入框"  onclick="window.location.href='Try.py'"/><br />
	</form>	
	<hr/>
	<div id="output">
	</div>
